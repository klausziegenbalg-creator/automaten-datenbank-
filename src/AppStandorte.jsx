...
function AppStandorte() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const [standorteSnap, automatenSnap, kontakteSnap] = await Promise.all([
          getDocs(collection(db, "Standorte")),
          getDocs(collection(db, "Automatenbestand")),
          getDocs(collection(db, "Kontakte")),
        ]);
        ...
        const kontakteItems = kontakteSnap.docs.map((d) => ({
          id: d.id,
          ...d.data(),
        }));
  ...
  const kontakteAmStandort =
    selected && selected.id
      ? kontakte.filter((k) => k.standortId === selected.id)
      : [];
      ...
      >
        {!selected ? (
          ...
          <>
            ...
            {/* Kontakte an diesem Standort - NEU */}
            <Card title="Kontakte an diesem Standort">
              <div
                style={{
                  border: `1px solid ${colors.border}`,
                  borderRadius: 12,
