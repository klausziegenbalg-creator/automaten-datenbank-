...
export default function AppDashboardReinigung() {
  ...
  // ---- Auth / Role / City Context ----
  const [authReady, setAuthReady] = useState(false);
  const [myRole, setMyRole] = useState(null);
  const [myCity, setMyCity] = useState(null);
  const isTeamleiter = myRole === "Teamleiter";
...
const [automaten, setAutomaten] = useState([]);
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (user) => {
      try {
        if (!user) {
          // Falls die App ohne Login geöffnet wurde
          setMyRole(null);
          setMyCity(null);
          setAuthReady(true);
          return;
        ...
        // user-doc: users/{uid} enthält { email, role, stadt }
        const uref = doc(db, "users", user.uid);
        const usnap = await getDoc(uref);

        const role = usnap.exists() ? (usnap.data()?.role ?? null) : null;
        const city = usnap.exists() ? (usnap.data()?.stadt ?? null) : null;

        setMyRole(role);
        setMyCity(city || null);

        // Für Teamleiter fix auf eigene Stadt (und keine "Alle Städte" Auswahl)
        if (role === "Teamleiter" && city) {
          setStadtFilter(city);
          setCenterFilter("Alle Center");
        }
      } catch (e) {
        console.error("Konnte User-Rolle/Stadt nicht laden:", e);
      } finally {
        setAuthReady(true);
      }
    });
  ...
  async function ladeWochenWartung(datumObj) {
    ...
    // Wir queryen über das Feld "datum" (Timestamp), weil dafür bereits Indizes existieren (stadt + datum).
    // Dadurch vermeiden wir neue Index-Kombinationen (stadt+woche, stadt+startedAt).
    const wwBase = collection(db, "wochenWartung");
    const wwCity = (isTeamleiter && myCity) ? where("stadt", "==", myCity) : null;
  ...
  async function ladeBestellungen() {
        const baseRef = collection(db, "bestellungen");
    const q = (isTeamleiter && myCity) ? query(baseRef, where("stadt", "==", myCity)) : baseRef;
  ...
  async function ladeDashboard() {
    setLoading(true);
    try {
            const automatenRef = collection(db, "automaten");
      const automatenQ = (isTeamleiter && myCity) ? query(automatenRef, where("stadt", "==", myCity)) : automatenRef;
      const automatenSnap = await getDocs(automatenQ);
      const alleAutomaten = automatenSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
      setAutomaten(alleAutomaten);

            const bestandRef = collection(db, "Automatenbestand");
      // Automatenbestand nutzt historisch oft das Feld "standort" (entspricht Stadt)
      const bestandQ = (isTeamleiter && myCity) ? query(bestandRef, where("standort", "==", myCity)) : bestandRef;
      const bestandSnap = await getDocs(bestandQ);
      ...
      if (stadtFilter !== "Alle Städte") {
        qReinigung = query(qReinigung, where("stadt", "==", stadtFilter));
      }
      if (centerFilter !== "Alle Center") {
        qReinigung = query(qReinigung, where("center", "==", centerFilter));
      }

      const reinigSnap = await getDocs(qReinigung);
      const reinigungenArr = reinigSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
      // Neueste zuerst (Index bleibt trotzdem einfach, weil orderBy in Firestore aufsteigend ist)
      reinigungenArr.sort((a, b) => {
        const aMs = a?.datum?.toMillis ? a.datum.toMillis() : (a?.datum ?? 0);
        const bMs = b?.datum?.toMillis ? b.datum.toMillis() : (b?.datum ?? 0);
        return bMs - aMs;
      });
      setReinigungen(reinigungenArr);

      await ladeWochenWartung(new Date(datum));

      const wartungsProtRef = collection(db, "Wartungsprotokolle");
      const wartungsProtQ = (isTeamleiter && myCity) ? query(wartungsProtRef, where("stadt", "==", myCity)) : wartungsProtRef;
      const wartungsProtSnap = await getDocs(wartungsProtQ);
  ...
  useEffect(() => {
    if (!authReady) return;
    // Für Teamleiter ohne Stadt: kein Query ausführen (würde sonst Permissions triggern)
    if (isTeamleiter && !myCity) return;
    ladeDashboard();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [authReady, myCity, myRole, datum, stadtFilter, centerFilter, wartungsAnsicht]);
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            <select
              ...
              style={{ border: `1px solid ${colors.border}`, borderRadius: 10, padding: "8px 10px", fontSize: 13, background: "#fff" }}
            >
              {!isTeamleiter && <option>Alle Städte</option>}
              {staedte.map((s) => (
