...
function AppAutomaten() {
  ...
  // Reinigungsteams laden (für Anzeige/Zuordnung)
  useEffect(() => {
...
function TeamSelectOrInfo({ editMode, label, formData, setFormData, teams }) {
  const currentId = formData.currentTeamId || "";
  const currentName = formData.currentTeamName || "";
  if (!editMode) {
    const show = currentName || (currentId ? (teams.find((t) => t.id === currentId)?.name || currentId) : "");
    return (
      <div style={{ minWidth: 200, marginBottom: 8 }}>
        <div style={{ fontSize: 12, color: colors.textMuted }}>{label}</div>
  ...
  const options = [{ value: "", label: "—" }].concat(
    (teams || []).map((t) => ({ value: t.id, label: t.name || t.id }))
  );
    ...
    <div style={{ minWidth: 200, marginBottom: 8 }}>
      ...
      <select
        ...
        onChange={(e) => {
          ...
          setFormData((p) => ({ ...p, currentTeamId: id, currentTeamName: name }));
        }}
      >
        {options.map((o) => (
