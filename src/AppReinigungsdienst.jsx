...
// Reload assignments after writes so UI immediately reflects changes (multi-Standort etc.)
async function reloadAssignments(teamId) {
  if (!teamId) return;
  try {
    const snap = await getDocs(collection(db, "team_assignments"));
    const items = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
    setAssignments(items.filter((a) => a.teamId === teamId));
  } catch (err) {
    console.error("Assignments reload:", err);
    // keep old state; show error in UI if desired
  }
}

// team_assignments für ausgewähltes Team
  ...
  // Standort-Zuordnung (Team -> Standort)
  const [assignStandortId, setAssignStandortId] = useState("");
  const [assignCityFilter, setAssignCityFilter] = useState("alle");
  const [assignStartDate, setAssignStartDate] = useState(todayISO());
  const [assignReason, setAssignReason] = useState("");

  // Automaten-Zuordnung: Stadt -> Standort -> Automaten
  // NOTE: cityFilter = "" bedeutet "keine Stadt gewählt".
  const [cityFilter, setCityFilter] = useState("ALL");
  const [standortFilterId, setStandortFilterId] = useState("ALL");
  ...
  useEffect(() => {
    ...
    return onSnapshot(
      ...
      (snap) => {
        ...
        if (!selected && rows.length && panel !== "create") {
          const first = rows[0];
          setSelected(first);
  ...
  // -------------------------
// Aktive Standorte (Team kann mehrere haben)
// -------------------------
const currentStandortAssignments = useMemo(
  () => assignments.filter((a) => a.entityType === "standort" && !a.endDate),
  [assignments]
);

const currentStandorte = useMemo(() => {
  const ids = new Set(currentStandortAssignments.map((a) => a.entityId).filter(Boolean));
  return standorte.filter((s) => ids.has(s.id));
}, [currentStandortAssignments, standorte]);
...
// -------------------------
// Standorte des ausgewählten Reinigungsteams (aktive Zuordnung) – und davon nur die, die Automaten haben
// NOTE: Für "Standort zuordnen" darf NICHT gefiltert werden (sonst kann ein neues Team nichts zuordnen).
// Der Filter "noch nicht zugewiesen" gilt NUR für Automat->Mitarbeiter Zuteilung und ist optional.
// -------------------------
const activeTeamStandortIds = useMemo(() => {
  if (!selected?.id) return new Set();
  const ids = new Set();
  assignments
    .filter((a) => a.teamId === selected.id && a.entityType === "standort" && !a.endDate)
    .forEach((a) => a?.entityId && ids.add(a.entityId));
  return ids;
}, [assignments, selected?.id]);

const teamStandorteMitAutomaten = useMemo(() => {
  if (!selected?.id) return [];
  return standorteMitAutomaten.filter((s) => activeTeamStandortIds.has(s.id));
}, [standorteMitAutomaten, activeTeamStandortIds, selected?.id]);

const teamStandorteNochNichtZugewiesen = useMemo(() => {
  if (!selected?.id) return [];

  // Map StandortId -> hat mindestens einen zugewiesenen Automaten?
  const standortHasAssignee = new Map();
  automatenbestand.forEach((a) => {
    const sid = a?.standortId;
    if (!sid) return;
    if (!activeTeamStandortIds.has(sid)) return;
    const has = Boolean((a?.cleaningAssigneeName || "").trim());
    if (has) standortHasAssignee.set(sid, true);
    else if (!standortHasAssignee.has(sid)) standortHasAssignee.set(sid, false);
  });

  // Für die UI ist es hilfreicher, ALLE Team-Standorte zu zeigen.
  // Die "noch nicht zugewiesen" Logik bleibt bestehen, wird aber NICHT mehr für City/Standort Dropdowns verwendet.
  return teamStandorteMitAutomaten.filter((s) => standortHasAssignee.get(s.id) !== true);
}, [automatenbestand, activeTeamStandortIds, teamStandorteMitAutomaten, selected?.id]);
  ...
  // -------------------------
  // City/Standort Dropdowns für "Automaten zuteilen":
  // - Primär: Standorte, die dem Team zugewiesen sind
  // - Fallback (wenn noch keine Standorte zugewiesen): alle Standorte mit Automaten
  const standorteForAutomaten = useMemo(() => {
    return teamStandorteMitAutomaten.length ? teamStandorteMitAutomaten : standorteMitAutomaten;
  }, [teamStandorteMitAutomaten, standorteMitAutomaten]);

  const cityOptions = useMemo(() => {
    const set = new Set();
    standorteForAutomaten.forEach((s) => {
      const city = String(s?.standort || "").trim();
      if (city) set.add(city);
    });
    const list = Array.from(set).sort((a, b) => a.localeCompare(b, "de"));
    return [{ value: "ALL", label: "Alle Städte" }, ...list.map((v) => ({ value: v, label: v }))];
  }, [standorteForAutomaten]);

  const standortOptionsByCity = useMemo(() => {
    const list = standorteForAutomaten
      .filter((s) => cityFilter === "ALL" || (s.standort || "").trim() === cityFilter)
      .map((s) => ({ value: s.id, label: `${s.centername || "—"} · ${s.standort || "—"}` }))
      .sort((a, b) => a.label.localeCompare(b.label, "de"));

    return [
      { value: "ALL", label: "Alle Standorte" },
      { value: "LAGER", label: "Lager / kein Standort" },
      ...list,
    ];
  }, [standorteForAutomaten, cityFilter]);

  useEffect(() => {
    setStandortFilterId("ALL");
  }, [cityFilter]);

  const automatenForSelectedStandort = useMemo(() => {
    const s = (automatSearch || "").trim().toLowerCase();

    // Grundregel: In der Zuteilungs-UI sollen Automaten aus den (Team-)Standorten gezeigt werden
    // (Fallback: alle Standorte mit Automaten) + Lager-Option.
    const allowedStandortIds = new Set(standorteForAutomaten.map((x) => x.id));
    let list = automatenbestand.filter((a) => !a?.standortId || allowedStandortIds.has(a.standortId));

    // Lager
    if (standortFilterId === "LAGER") {
      list = automatenbestand.filter((a) => !a?.standortId);
    } else if (standortFilterId && standortFilterId !== "ALL") {
      list = automatenbestand.filter((a) => a?.standortId === standortFilterId);
    } else if (cityFilter && cityFilter !== "ALL") {
      // Stadt gewählt, aber Standort "Alle" -> nach Stadt filtern
      const allowed = new Set(
        standorteForAutomaten.filter((st) => (st.standort || "").trim() === cityFilter).map((st) => st.id)
      );
      list = automatenbestand.filter((a) => a?.standortId && allowed.has(a.standortId));
    }
    ...
    return list.filter((a) => {
      const text =
        (pickFirst(a, ["maschinenCode", "automatCode", "seriennummer", "model", "hersteller"], "") || "") +
        " " +
        (a.centername || "") +
        " " +
        (a.standort || "");
  ...
  }, [automatenbestand, standorteForAutomaten, standortFilterId, cityFilter, automatSearch]);
  ...
  async function createTeam() {
    ...
    try {
      ...
      console.error("Reinigungsteam anlegen:", err);
      setSaveError(err?.message || String(err));
    } finally {
      setSavingTeam(false);
    }
  ...
  // -------------------------
  const assignCityOptions = useMemo(() => {
    const set = new Set();
    // Für "Standort zuordnen": alle Standorte mit Automaten anbieten
    standorteMitAutomaten.forEach((s) => {
      const city = (s.standort || "").trim();
      if (city) set.add(city);
    });
    return [
      { value: "alle", label: "Alle Städte" },
      ...Array.from(set)
  ...
  const standortOptionsForAssign = useMemo(() => {
    const list = standorteMitAutomaten
      .filter((s) => assignCityFilter === "alle" || (s.standort || "").trim() === assignCityFilter)
      .map((s) => ({ value: s.id, label: `${s.centername || "—"} · ${s.standort || "—"}` }))
      .sort((a, b) => a.label.localeCompare(b.label, "de"));
    return [{ value: "", label: "Bitte wählen…" }, ...list];
  }, [standorteMitAutomaten, assignCityFilter]);
    ...
    try {
      ...
      await addDoc(collection(db, "team_assignments"), {
        ...
        updatedAt: serverTimestamp(),
            ...
            await reloadAssignments(selected.id);
    ...
    try {
      ...
      await updateDoc(doc(db, "Automatenbestand", automat.id), {
        ...
        cleaningAssignedAt: serverTimestamp(),
	      ...
	      // ✅ Legacy/Kompatibilität: auch in "reinigungsdienst_automaten" schreiben
	      // Wichtig: Code/DocId genau einmal ermitteln (sonst redeclare-Scope-Fehler)
	      const rdAutomatCode =
	        (automat.automatenCode || automat.automatCode || automat.maschinenCode || automat.id || "").toString();
	      const rdDocId = rdAutomatCode || automat.id;
	      await setDoc(
        doc(db, "reinigungsdienst_automaten", rdDocId),
        {
          automatCode: rdAutomatCode,
          teamId: selected.id,
          teamleiter: ass.cleaningAssigneeName || (selected?.leader?.name || selected?.teamleiter || ""),
      ...
      <div style={{ display: "grid", gridTemplateColumns: "420px 1fr", gap: 12 }}>
        ...
        >
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <Input label="Suche" value={search} onChange={setSearch} placeholder="Reinigungsteam / Ansprechpartner…" />
            <Select
              label="Status"
              value={filterStatus}
        ...
        {/* RIGHT */}
        <Card
          title={selected ? `Team: ${selected.name || "—"}` : "Reinigungsteam anlegen"}
          right={
              ...
              <Card title="Übersicht">
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  ...
                  <div>
                    ...
                    <div style={{ marginTop: 4, color: colors.textMuted, fontWeight: 800, fontSize: 12 }}>
                      Aktive Standorte (Anzahl): {currentStandorte.length}
                    </div>
                  </div>
                </div>
              </Card>

<Card title="Aktive Standorte">
  {currentStandorte.length ? (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {currentStandorte.map((s) => (
        <div
          key={s.id}
          style={{
            border: `1px solid ${colors.border}`,
        ...
        >
          ...
          <Button
            variant="danger"
            title="Standort vom Team trennen (setzt Enddatum im Assignment)"
            onClick={async () => {
              const ass = currentStandortAssignments.find((a) => a.entityId === s.id && !a.endDate);
              if (!ass?.id || !selected?.id) return;

              const sicher = window.confirm(
                `Standort "${s.centername || "—"} · ${s.standort || "—"}" wirklich entfernen?`
              );
              if (!sicher) return;
              ...
              try {
                ...
                await updateDoc(doc(db, "team_assignments", ass.id), {
                  endDate: todayISO(),
                  ...
                  updatedAt: serverTimestamp(),
                                              ...
                                              await reloadAssignments(selected.id);
// optional: Standort currentTeamId leeren, wenn es noch auf dieses Team zeigt
                if ((s.currentTeamId || null) === selected.id) {
                  await updateDoc(doc(db, "Standorte", s.id), {
                    currentTeamId: null,
                    updatedAt: serverTimestamp(),
                  });
                }
              } catch (err) {
                console.error("Standort entfernen:", err);
                setSaveError(err?.message || String(err));
              } finally {
                setSavingTeam(false);
              }
    ...
    <div style={{ color: colors.textMuted, fontSize: 13 }}>Keine aktiven Standorte zugeordnet.</div>
  )}
</Card>
              ...
              <Card
                ...
                right={
                  <Button variant="ok" disabled={savingTeam} onClick={createTeam}>
                    {savingTeam ? "…" : "Reinigungsteam anlegen"}
                  </Button>
                }
              >
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          {panel === "edit" && selected ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <Card
                title="Reinigungsteam bearbeiten"
                right={
          ...
          {panel === "assignStandort" ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <Card
                title="Standorte zuordnen (nur mit Automaten)"
                right={
  ...
  onChange={(v) => {
    setAssignCityFilter(v);
    setAssignStandortId("");
  }}
  options={assignCityOptions}
/>
              ...
              <Card title="Automaten zuteilen (Stadt → Standort → Automat)">
                {!selected ? (
                  ...
                  <>
                    ...
                    {!standortFilterId ? (
                      <div style={{ color: colors.textMuted, fontSize: 13 }}>Bitte zuerst eine Stadt und einen Standort wählen. (Hinweis: Es werden nur Standorte angezeigt, die diesem Team aktiv zugeordnet sind.)</div>
                    ) : (
                              ...
                              >
                                <div style={{ minWidth: 260 }}>
                                  ...
                                  <div style={{ marginTop: 6, display: "inline-flex", alignItems: "center", gap: 8 }}>
                                    <span style={{ fontSize: 12, fontWeight: 800, color: colors.textMuted }}>Reinigungsteam:</span>
                                    <span
                                      style={{
                                        fontSize: 12,
                                        fontWeight: 900,
                                        ...
                                        borderRadius: 999,
                                        border: `1px solid ${colors.border}`,
                                        background: a.cleaningTeamId ? "#e3f2fd" : "#fff3e0",
                                        color: colors.textMain,
                                      }}
                                    >
                                      {a.cleaningTeamId ? (a.cleaningTeamName || a.cleaningTeamId) : "nicht zugewiesen"}
                                    </span>
                                  </div>
                                  </div>
